...
(define (text/cselim text [operators default-object])
  ;; (expression->text (expression/cselim (text->expression text)))
  ;; Like ^ but avoids copying.
  (let ((lam (lam/make #f)))
    (parameterize ((*warn-body?* (*allow-warnings?*))
                   (*global-lam* lam)
      ...
      (when (*recompute-input-sets?*)
        (collect-input-set! (lam/node (*global-lam*))))
      (sort-subexpressions! (lam/node (*global-lam*)))
      (when (*alpha-rename-lazily?*)
...
(define (expression/copy expression)
  (parameterize ((*associations* (table+/make)))
...
(define (expression/cselim expression [operators default-object])
  (with-copied-expression expression
    (lambda ()
      (parameterize ((*operators*
                      (process-operators
...
(define (lambda-expression->procedure expression [environment default-object])
  (eval (expression->text expression)
        (if (default-object? environment)
            (nearest-repl/environment)
            environment)))

;node is not defined in this expression...!!! (var-node/name node)
...
(define (expression/walk expression recvr)
  (let ((context (context/make))
    ...
    (when (not (expression/lazy-rename? expr))
...
(define *allow-warnings?* (make-parameter #t))
...
(define *recompute-input-sets?* (make-parameter #t))
(define *alpha-rename-lazily?* (make-parameter #t))
...
(define *warn-body?* (make-parameter undefined-value))
(define *operators* (make-parameter undefined-value))

(define *global-lam* (make-parameter undefined-value))
(define *global-env* (make-parameter undefined-value))
(define *global-vars* (make-parameter undefined-value))
(define *constants* (make-parameter undefined-value))

(define *associations* (make-parameter undefined-value))
(define *reverse-associations* (make-parameter undefined-value))

(define (collect-expression)
...
(define (with-expression expression thunk)
  (parameterize ((*alpha-rename-lazily?* (expression/lazy-rename? expression))
...
;;;; Data structures
(struct <expression> (lazy-rename? constants variables graph) #:transparent #:mutable)
(define (expression/make [lazy-rename? #f] [constants '()] [variables '()] [graph #f])
  (<expression> lazy-rename? constants variables graph))
(define expression/lazy-rename? <expression>-lazy-rename?)
(define expression/constants    <expression>-constants)
(define expression/variables    <expression>-variables)
(define expression/graph        <expression>-graph)
#;(define-structure (j-expression
    ...
    (lazy-rename? #f read-only true)
    (constants '() read-only true)
    (variables '() read-only true)
    (graph #f read-only true))

(define expression-unparse-fields
  `("expression" ("lazy-rename?" ,expression/lazy-rename?)
                 ("graph" ,expression/graph)))

(define (expression/body expr)
  (lam/body (expression/graph expr)))

(define (expression/node expr)
  (lam/node (expression/graph expr)))

(struct <context> (constants free-vars bound-vars result-cache) #:transparent #:mutable)
(define (context/make [constants (table+/make)]  [free-vars (table+/make)]
                      [bound-vars (table+/make)] [result-cache (table+/make)])
  (<context> constants free-vars bound-vars result-cache))
(define context/constants   <context>-constants)
(define context/free-vars   <context>-free-vars)
(define context/bound-vars  <context>-bound-vars)
(define context/result-cache <context>-result-cache)
...
(struct <node> (type1 type2 extra children parents input-set generation name mark) #:transparent #:mutable)
(define (node/make [type1 #f] [type2 #f]
                   [extra #f] [children '()] [parents '()] [input-set '()] [generation 0] [name #f]
                   [mark #f])
  (<node> type1 type2 extra children parents input-set generation name mark))
(define node/type1      <node>-type1)
(define node/type2      <node>-type2)
(define node/extra      <node>-extra)
(define node/children   <node>-children)
(define node/parents    <node>-parents)
(define node/input-set  <node>-input-set)
(define node/generation <node>-generation)
(define node/name       <node>-name)
(define node/mark       <node>-mark)
(define set-node/extra!      set-<node>-extra!)
(define set-node/children!   set-<node>-children!)
(define set-node/parents!    set-<node>-parents!)
(define set-node/input-set!  set-<node>-input-set!)
(define set-node/generation! set-<node>-generation!)
(define set-node/name!       set-<node>-name!)
(define set-node/mark!       set-<node>-mark!)
#;(define-structure (node (conc-name node/)
                          ...
                          (constructor
                           node/make
                           (type1 type2 extra [children default-object])))
    (type1 #f read-only true)
    (type2 #f read-only true)
    ...
    (parents '() read-only #f)
    (input-set '() read-only #f)
    (generation 0 read-only #f)
    (name #f read-only #f)
...
(struct <var> (name lam) #:transparent #:mutable)
(define (var/make [name #f] [lam #f]) (<var> name lam))
(define var/name <var>-name)
(define var/lam  <var>-lam)
(define set-var/name! set-<var>-name!)
(define set-var/lam!  set-<var>-lam!)
...
(define (var-node/name var)
  (var/name (node/extra var)))

(define (set-var-node/name! var name)
  (set-var/name! (node/extra var) name))

(define (var-node/lam var)
  (var/lam (node/extra var)))

(define (set-var-node/lam! var lam)
  (set-var/lam! (node/extra var) lam))

;;;; Data structures (continued)
(struct <lam> (parent params body node aux text) #:transparent #:mutable)
(define (lam/make [parent #f] [params '()] [body #f] [node #f] [aux #f] [text #f])
  (<lam> parent params body node aux text))
(define lam/parent <lam>-parent)
(define lam/params <lam>-params)
(define lam/body   <lam>-body)
(define lam/node   <lam>-node)
(define lam/aux    <lam>-aux)
(define lam/text   <lam>-text)
(define set-lam/parent! set-<lam>-parent!)
(define set-lam/params! set-<lam>-params!)
(define set-lam/body!   set-<lam>-body!)
(define set-lam/node!   set-<lam>-node!)
(define set-lam/aux!    set-<lam>-aux!)
...
(define (lam-node/params lam)
  (lam/params (node/extra lam)))

(define (lam-node/body lam)
  (lam/body (node/extra lam)))

(define (set-lam-node/body! lam body)
  (set-lam/body! (node/extra lam) body))

(define (lam-node/aux lam)
  (lam/aux (node/extra lam)))

(define (global-lam/make)
  (lam/make #f))

(define (global-lam? lam)
  (eq? #f (lam/parent lam)))

(define (constant-node/value lit)
  (node/extra lit))
...
(define (add-global-variable! name node)
  ...
  (*global-vars* (cons node (*global-vars*)))
  node)
              ...
              (grow-environment
               ...
               (%map-2 (lambda (name value)
                         (let ((node (->graph value environment olam)))
                           ...
                           (when (not (eq? (node/type1 node) 'INPUT))
...
(define (combination->graph elements environment olam)
  (let ((operator (->graph (car elements) environment olam))
    ...
    (let* ((children (cons operator operands))
           (new-node (node/make 'REDUCIBLE 'COMBINATION #f children)))
      (for-each (lambda (child)
                  (set-node/parents!
                   child
                   (cons new-node (node/parents child))))
                children)
      (set-node/extra! new-node #f)
      new-node)))

;;; Environment utilities

(define (j-lookup environment var)
  (let ((place (assq var environment)))
    (and place
         (cdr place))))

(define (grow-environment environment names values)
  (foldr (λ (n v acc)
           (list* (cons n v) acc))
         environment
         names
...
(define (j-operator? node)
  (assq node (*operators*)))

(define (can-reorder? node)
...
(define (with-aux lam recvr)
  (if (lam/aux lam)
      ...
      (let ((old-node (lam/node lam))
        ...
        (let* ((new-body (node/make 'REDUCIBLE 'COMBINATION #f
                                    (list old-node)))
...
(define (graph->expression graph)
  (when (not (eq? (node/type graph) 'LAMBDA))
...
(define (operator&not-only-operand? node parent)
...
(define (update-children! parent new-children)
  ...
  (when (eq? (node/type parent) 'LAMBDA)
    (begin
      (when (or (null? new-children)
...
(define (object-copier %copy copy-fields!)
  (lambda (object)
...
(define (%unify-1 expressions)
  (let ((global-lam* (lam/make #f))
    ...
    (for-each (lambda (expr)
              ...
              expressions)
    (expression/make #t constants* vars* global-lam*)))
...
(define (%expression/bind expression bindings)
  (split-list
   ...
   (lambda (bound unaffected)
     (let* ((expr*
             (%unify-1 (cons (expression/make #t
                                              (expression/constants expression)
                                              unaffected
                                              (expression/graph expression))
                             (%map-1 cadr bindings))))
...
(define (object-comparator %comparator)
  (lambda (obj1 obj2)
...
(define (constant-value node)
  (constant-node/value node))

(define (variable-name node)
  (var-node/name node))

(define (lambda-parameters node)
  (lam-node/params node))

(define (lambda-body node)
  (lam-node/body node))

(define (combination-operator node)
  (car (node/children node)))

(define (combination-operands node)
...
(define (make-interner selector maker)
...
(define (build-variable name)
  (node/make 'INPUT 'VARIABLE (var/make name #f)))
    ...
    (cond ((not (eq? (node/type node) 'LAMBDA))
          ...
          ((lam/parent (node/extra node))
           ...
           (lambda (current-parent)
             (let ((lam* (node/extra node)))
               (when (not (eq? current-parent lam))
                 (let ((real-parent
