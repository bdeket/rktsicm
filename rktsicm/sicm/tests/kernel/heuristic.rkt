  ...
  (test-suite
   ...
   (test-case
    ...
    (check-equal? (expression (heuristic-canonicalize-complex (make-rectangular (angle -1) (exp 1)) #t))
                  '(+ :pi (* 0+1i (exp 1))))
    (check-equal? (expression (heuristic-canonicalize-complex (make-rectangular n:phi 0.5772156649015329)))
                  '(+ :phi (* 0+1i :euler)))
    ...
    (check-equal? (expression (heuristic-canonicalize-complex (make-polar (sqrt 3) (* 1/3 n:pi))))
