...
(define the-tests
  (test-suite
   ...
   (test-case
    ...
    (check-equal? (symb:zero? 'a) `(= ,n:zero a))
    (check-equal? (symb:one? 1) #t)
    (check-equal? (symb:one? 2) #f)
    (check-equal? (symb:one? 'a) `(= ,n:one a))
    )
   ...
   (test-case
    ...
    (check-equal? (symb:cos n:pi/2) 0.)
    (check-equal? (symb:cos n:pi) -1.)
   ...
   (test-case
    ...
    (check-equal? (symb:tan n:pi/4) 1.)
    (check-equal? (symb:tan (+ n:-pi/4 n:pi)) -1.)
    (check-exn #px"Undefined -- TAN" (λ () (symb:tan n:pi/2)))
